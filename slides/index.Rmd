---
title: "Applied regression modelling with R"
institute: "Department of Animal & Veterinary Sciences · Aarhus University"
author: "Gavin Simpson"
date: "0900&ndash;1600 CET 14<sup>th</sup>&ndash;16<sup>th</sup> April 2026"
output:
...
> Models encapsulate understanding of a biological process into a deterministic process

* Mathematical models
* Simulation models

> Estimating effects, testing hypotheses, interpreting data
...
Equations derived from basic principles & scientific understanding

Mathematical description &rarr; simplification &rarr; general predictions
...
\frac{dN_2}{dt} & = r_2N_2\left ( 1 - \frac{N_2}{K_2} - \frac{\alpha_{12}N_1}{K_2} \right )
\end{align}
...
A set of interacting modules that encapsulate physical or biological processes

Parameterised using literature values & run on powerful computers

* General Circulation Models
* NASA amopsheric CO~2~ 

```{r sim-model, fig.align="center", out.width="50%", echo=FALSE}
...
Fit statistical models to data &rarr; estimate parameters &rarr; assess uncertainty

Generally used to test **scientific hypotheses** operationalised as a statistical model

```{r statistical-model-example, echo = FALSE, fig.align="center", out.width="60%"}
set.seed(1)
...
We will stick to frequentist (or likelihood-ist really) for this course
...
The linear model behind the data we saw a couple of slides ago was

$$ \hat{y}_i = 2 + (0.85 \times x_i) $$

This is the equation of a straight line, more generally

$$ \hat{y}_i = b + (a \times x_i) $$

$$ \hat{y}_i = \beta_0 + \beta_1 x_i $$

> Q: What values can this model predict for $y_i$?

---

# Linear models

The problem with using linear models is that

1. They can predict values for $y_i$ that are biologically nonsensical,

2. They fail to capture the mean-variance relationships commonly encountered in biological data

We could transform $y_i$ but then we would be modelling something other than $\hat{y_i}$, which statisticians call the expected value of $Y$

$$ \mathbb{E}(y_i) $$

Usually we are interested in $\mathbb{E}(y_i)$, not $\mathbb{E}(\log(y_i))$

---

# Generalized linear models

Generalised linear models (GLMs) are an extension of linear regression & include Poisson, logistic and other regression models
...
The binomial and Poisson distributions are part of a larger family **exponential family**

Also include the Gaussian distribution

GLMs *sensu stricto* were created for members of the **exponential [dispersion] family**

General ideas we will discuss today extend to distributions beyond this family
...
And assumed properties of residuals

$$ \varepsilon_i \sim \text{Normal}(0, \sigma^2) $$

This doesn't work out the same for GLMs &mdash; we don't have a residual term $\varepsilon$ in the linear predictor
...
Is this really necessary? Is it just some mathematical sleight of hand?

Yes, it is necessary (except for a couple of special cases Gaussian, Binomial)

No, it is not just sleight of hand

> Focus on the distribution of each observation of $y_i$

What values can each observation take?
...
```
m1 <- glm(log(y) ~ x, data = my_data, family = gaussian())
...
```{r, fig.width = 6, fig.align="center", out.width="75%"}
...
```{r, fig.width = 6, fig.align="center", out.width="75%", cache = FALSE}
library("gglm")
moth_glm1 |> gglm()
```
...
```{r load-performance, cache = FALSE, fig.width = 1.777777 * 12, fig.height = 12, out.width = "85%", fig.align = "center"}
library("performance")
...
This also applies to categorical terms as formally (with treatment coding) we're changing 1 unit (0 to 1) when we flick the switch (off to on)
...
These will become the same once we ask for comparisons/slopes conditioned on `sex` and `marrow` as now there is nothing else to average in the AME
...
We are averaging the comparisons of the levels of `marrow` over the other variables in the model (`sex`, `death`)
...
- variation *within* a block&mdash; $\sigma^2_{\mathsf{within}}$
    - between technical reps within a single biological rep
- variation *between* blocks&mdash; $\sigma^2_{\mathsf{between}}$
    - between biological reps
...
- random sample of biological reps from set of all biological reps
...
# New styles random effects

A lot of what is written about random effects represents a traditional, frequentist philosophy

Often refer to these as *old style random effects*

Lots of things also look, walk, and quack like a random effect --- e.g. penalized splines

They have the same mathematical form

These are *new style random effects*

---

# New styles random effects

This *new style* philosophy essentially states that we use the random effects as a convenience

Convenience is to estimate things of interest

*Penalized splines* are random effects, but we don't think of them as random draws from a population of possible splines

--

Basically, we can relax a little about some of the formal requirements for ranodm efects

Especially if using them to account for things not of primary interest

---

# New style random effects

The main exception is one of practicality

Most mixed model software is formulated from the classic random effects model

Estimating a variance &mdash; that requires sufficient observations

Hence the rule about minimum 5 or 6 levels in the random effect variable
...
$a_i$ intercepts & $b_i$ slopes are random with covariance $\sigma_{uv}$
...
`1` means an intercept &mdash; not strictly necessary in the fixed effects part
...
More simply, once you understand the recipe:

```{r chick-pop-level-eff}
...
Or, more simply with `plot_predictions()`

```{r chick-pop-plot-predictions-lmm2}
...
We remove the population-level effect of `Diet` from the previous model'a $a_i$

???

Note that $\alpha_1 \mathtt{Diet}_i$ is removed from the $a_i$ line
...
.col-6[
```{r predictions-chick-lmm3, echo = FALSE, cache = FALSE}
```
]
]
...
```{r chick-pop-plot-predictions-lmm3}
...
For strict compatibility with a repeated measures ANOVA we need what is known as the *maximal random effects model*

We have the same formula in the random effects as the fixed effects
...
You can then peel off a layer from the random effects using AIC or LRT if you encounter warnings while fitting (as we do here)

Barr *et al* (2013) J. Mem. Lang. 68. <https://doi.org/10.1016/j.jml.2012.11.001>
...
As GAMs can be seen as GLMMs, you can fit GLMMs using *mgcv*
...
Code to fit this model in `lmer()` and `gam()` is in `rat-hormone-example.R`
...
```{r plot-gamm-dharma, fig.align="center", out.width="75%"}
library("mgcv")
...
```{r chick-gam-model-1, fig.align="center", out.width="75%"}
...
```{r chick-gam-model-2, fig.align="center", out.width="75%"}
...
```{r chick-gam-model-3, fig.align="center", out.width="75%"}
...
# Model diagnostics plots for MM

```{r model-diagnostics-glmm, fig.width = 1.777777 * 12, fig.height = 12, out.width = "85%", fig.align = "center"}
library("performance")
...
Copyright © (2015&ndash;2026) Gavin L. Simpson Some Rights Reserved
